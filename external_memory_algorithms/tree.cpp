...
unsigned find_root() {
    ...
    for (unsigned i = 0; i < 3 * list_size; i += n_read) {
        ...
        fwrite(c, 4, n_read, tempfile);
    }
    fprintf(stderr, "Root wasn't found\n");
    exit(2);
}
...
void add_root(FILE* file) {
    ...
    triple[1] = root;
    triple[2] = 0;
    fwrite(triple, 4, 3, file);

    list_size++;
}

void print_results(const char* ofilename) {
    block_size = RAM_SIZE / 5 * 3;
    a = memory;
    b = memory + block_size;
    ...
    for (unsigned i = 0; i < 3*list_size; i += block_size) {
        n_read = (unsigned) fread(a, 4, block_size, tempfile);
        for (unsigned j = 0; j < n_read / 3; ++j) {
            b[2*j] = a[3*j];
            b[2*j+1] = a[3*j+2];
        }
        fwrite(b, 4, n_read / 3 * 2, ofile);
    }
    fclose(ofile);
}
...
void ranking() {
    ...
    merge_sort(tempfile, sortedfile1, 1);
    merge_sort(sortedfile1, sortedfile2, 2);

    root = find_root();
    add_root(sortedfile2);
    merge_sort(sortedfile2, sortedfile1, 1);
    merge_sort(sortedfile1, sortedfile2, 2);

    cyclic_join();
    merge_sort(joinedfile, tempfile, 1);
